...
1. After the one-time frontend choice, call `start_design_session` with the creator's natural-language brief, the clear current conversation language, and the selected `frontend_mode`. Default to nine independent 9:16 high-quality image requests, a live-action photography Rendering Contract, and `chaptered_variation` unless the brief explicitly asks for one continuous scene/look/event. Treat 2160×3840 as a requested delivery target only; Codex-managed generation does not guarantee returned pixel dimensions or format.
