...
## Choose automatic or guided authoring at the start

At the start of every new or resumed creative workflow, ask once “选择创作方式？” and offer “全自动创作（推荐）” or “逐步确认”. In English ask “Choose an authoring mode” and offer “Automatic (Recommended)” or “Guided review”. Skip the question when the creator already made the choice in the same request.

Pass `authoring_mode: automatic` for the first choice and `authoring_mode: guided` for the second. Automatic mode is the creator's up-front approval for APSAL to use the complete brief, select the highest-ranked valid DNA for every required type, confirm all five layers and thirteen protocol roles in order, and finalize the Prompt/Skill package without per-layer or final-overview confirmation. It must still preserve the same identity, rights, continuity, output and QA rules. The resulting decisions use the traceable source `automatic_default`; never claim that the creator individually confirmed them.

If real reference images are supplied, resolve their role, allowed/forbidden uses, applicable Jobs, rights/consent and redistribution state before starting, then pass them as start-time `reference_bindings`. Missing or ambiguous rights, an ambiguous interface language, a missing required DNA type, protocol incompatibility, or a failed validation pauses automatic mode with one concrete recovery request. Do not silently drop the reference, weaken validation, or fall back to guided mode.

When automatic start returns `state: ready`, do not call `present_element_layer`, `commit_element_layer`, or `finalize_theme` again. Summarize the chosen direction, five completed layers, nine-shot plan, package path and SHA-256, then offer the normal generation/package delivery choice. Automatic authoring removes design confirmations; image generation remains a separate delivery action and still follows the one-Job-per-turn contract.

## Ask before opening the optional Studio projection

At the start of each new or resumed creative workflow, ask once before calling `start_design_session`, unless the creator already made the choice in the same request. In Chinese ask exactly “是否同时打开 APSAL Studio 前端？” and offer “打开并联动（推荐）” or “仅在 Codex 中继续”. In English ask “Open the APSAL Studio frontend for this creation?” and offer “Open and link (Recommended)” or “Continue in Codex only”. This is a frontend choice, not a language or generation confirmation.
...
2. In guided mode, for each layer call `present_element_layer`. Never skip or hide a protocol role:
   ...
   - `worldbuilding`: Subject + World + Look. Call `recommend_layer_dna` for Character and Environment DNA; explicitly confirm identity, space, time, wardrobe, grooming, props and ownership. Chaptered Variation defaults to nine distinct scene designs and nine distinct makeup/hair/wardrobe combinations, one complete design per Job. The same facial identity is the continuity lock. Continuous Narrative defaults to one core scene and one confirmed look.
   - `narrative`: Event + Sequence. Recommend Composition and Shot DNA; show the sequence strategy and nine distinct Scene cards with consequences and order. Every Job must have a different action-led body state, hand function, gaze motivation, face orientation, body orientation, expression and weight distribution.
   - `image`: Camera + Light + Style + Color/Post. Recommend Style and Lighting DNA; explicitly confirm viewpoint, coverage, optics, composition, light source/direction/quality/contrast, palette, temperature, saturation, curve, grain, sharpness, dynamic range and skin-tone policy. The default nine-Job plan uses nine different focal lengths and nine different physically motivated lighting designs. Compile the observable perspective purpose as well as the nominal focal length.
...
3. In guided mode, treat every element card as a complete proposal. Fill the card itself with the recommendation, why it fits the current brief, adjustable directions, concrete values, observable effects, locks and QA expectations; do not leave proposal sections blank and do not move substantive suggestions into surrounding chat. Accept a natural-language revision to one element, show its downstream effects, and preserve all non-target locks.
   ...
   - Styling versatility never means identity drift. Preserve facial geometry, facial features, adult age, natural skin identity and body proportions. In Chaptered Variation, makeup, hair and wardrobe change for every Job; these changes must never replace the face or the person. In Continuous Narrative, keep one confirmed look unless an explicit event motivates a change.
...
5. In guided mode, call `commit_element_layer` only after every element in that layer is explicitly confirmed. In automatic mode the start operation performs these commits itself; do not repeat them. Never call legacy `commit_stage` for a new five-layer session.
...
8. In guided mode at `review_pending`, show all thirteen confirmed decisions plus a compact nine-shot overview. Include the chosen set strategy and, for every Job, its scene, makeup, hair, wardrobe, action, pose, hands, gaze, face orientation, body orientation, expression, composition, focal length, perspective purpose and lighting. State separately that facial identity remains locked.
9. In guided mode call `finalize_theme` after the creator confirms the overview. Automatic mode returns the same deterministic package directly from `start_design_session`. The package contains nine different positive, negative and full Prompts, the actual permitted reference images under `assets/references/`, a per-Job mapping in `references/run_manifest.json`, five-stage semantic thumbnails, QA, checksums, and equally complete English and Chinese guides. Show its path and SHA-256 to the creator. Treat it as the source of truth.
