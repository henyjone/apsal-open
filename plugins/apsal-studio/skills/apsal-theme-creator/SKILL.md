...
8. In guided mode at `review_pending`, show all thirteen confirmed decisions plus a compact nine-shot overview. Include the chosen set strategy, per-Job scene, look, action/body state, framing, focal perspective and the invariants that bind the set together.
9. In guided mode call `finalize_theme` after the creator confirms the overview. Automatic mode returns the same deterministic package directly from `start_design_session`. The package contains all positive, negative and full Prompts, actual permitted references, five-stage semantic thumbnails, QA, checksums, and equally complete English and Chinese guides. Show its path and SHA-256 to the creator. Treat it as the source of truth.
