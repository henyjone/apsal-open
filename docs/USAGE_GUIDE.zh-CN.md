...
```text
apsal-studio@apsal-open  installed, enabled  0.13.0
```
...
APSAL Studio 默认跟随当前 Codex 对话，不在安装后强制显示语言选择页。中文创作描述建立中文会话，英文创作描述建立英文会话。0.13.0 的中文卡片只显示面向创作者的中文：内部角色名、层编号、字段名、状态、来源、资源层级、资源编号、质量状态和语义标签都留在后台，不再出现在卡片中。
...
每一层都会显示纯文字卡片，包含选择值、为什么这样设计、会影响什么、必须保持什么，以及如何检查成功。“可调整方向”现在都是可以直接点击的按钮；点击后先更新该元素并说明下游影响，不会跳过本层确认。你也可以回复：
...
从 0.9.0 开始不需要自己解压旧包；0.10.0 提供完整中英文操作说明；0.11.0 统一中文卡片与重要文字高亮；0.12.0 将完整设计提案和验收标准都放入卡片；0.13.0 增加章节式丰富变化、连续叙事和可点击方向。把压缩包附到 Codex，输入：
...
| Codex 开始向你解释结构化文件 | 确认插件为 0.13.0；对旧包直接说“打开这个 APSAL 包并生成第一张图” |
| 英文输入仍显示中文卡片 | 升级到 0.13.0 后打开新任务，或直接要求切换到英文 |
| 中文卡片仍出现英文内部字段 | 确认插件为 0.13.0 并打开新任务；新版中文呈现层会隐藏内部编号和机器标签 |
| 元素卡只显示标题或有空白区域 | 确认插件为 0.13.0 并新建任务；新版卡片应同时显示提案、理由、可点击方向、取值、预期结果、保持项和验收标准 |
| 九张图总在重复同一场景、妆造、姿势或焦段 | 新建 0.13 会话并保留默认“章节式丰富变化”，或在第一张卡从“连续叙事”切换；确认三场景、三妆造、九种身体状态和多焦段计划后再继续 |
