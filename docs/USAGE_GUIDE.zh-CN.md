...
```text
apsal-studio@apsal-open  installed, enabled  0.14.0
```
...
| Codex 开始向你解释结构化文件 | 确认插件为 0.14.0；对旧包直接说“打开这个 APSAL 包并生成第一张图” |
| 英文输入仍显示中文卡片 | 升级到 0.14.0 后打开新任务，或直接要求切换到英文 |
| 中文卡片仍出现英文内部字段 | 确认插件为 0.14.0 并打开新任务；新版中文呈现层会隐藏内部编号和机器标签 |
| 元素卡只显示标题或有空白区域 | 确认插件为 0.14.0 并新建任务；新版卡片应同时显示提案、理由、可点击方向、取值、预期结果、保持项和验收标准 |
| 五阶段没有缩略图 | 确认插件为 0.14.0 并打开新任务；DNA 选择卡保持纯文字，元素卡上方才显示五阶段缩略图 |
