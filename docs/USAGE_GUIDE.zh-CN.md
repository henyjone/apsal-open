...
→ 全自动：APSAL 选择首选 DNA、完成五层十三元素并打包
  或逐步确认：查看并修改每层设计方案
→ 十三元素与九镜头总览
→ 自动生成 Prompt/Skill 包
→ Codex 每回合生成一张图
...
```text
apsal-studio@apsal-open  installed, enabled  0.16.0
```
...
- **全自动创作（推荐）**：依据完整创作描述选择每层匹配度最高且权利有效的 DNA，按顺序完成五层十三元素，并直接生成 Prompt/Skill 包；不再逐层要求“确认”。
- **逐步确认**：保留原有工作流卡片，逐层查看、调整和确认，适合需要精细控制人物、世界、叙事与摄影语言的项目。

全自动方案会在项目中标记为“全自动方案”，不会伪装成创作者逐项确认。语言不明确、参考图用途或权利不清、必需 DNA 缺失、协议不兼容或结构验证失败时才暂停，并只询问解决当前问题所需的信息。图片生成仍是打包后的独立交付动作。
...
联动成功后是双向的：Codex 提出的修改会进入 Studio 的“Agent 联动”页；你也可以选中 Studio 工作流卡片，在“属性”页点击“编辑”，修改元素意图或属性，再点击“发送给 Codex”。这次编辑会成为带 revision 和字段差异的待处理提案，不会直接覆盖项目。Codex 在下一次 APSAL 操作前读取这条更新，并继续确认、调整、应用或拒绝。双方始终读写同一个 `.apsal/project.json` 内核。

Studio 的“当前片场”区域会显示本次会话是“全自动创作”还是“逐步确认”。全自动模式完成后，五层工作流仍完整显示，方便检查每个元素、九镜计划与最终状态。
...
参考图用途：（identity / subject / world / space / composition / lighting / wardrobe / color / style / prop）
```
...
1. **章节式丰富变化（推荐）**：九张分成三章，每章三镜，但九镜分别使用九个不同场景、九套不同妆容/发型/服装、九种姿态与动作、九种手部职责、眼神与面部朝向、九种构图、九个焦段和九套有物理来源的灯光。只锁定同一个人物的面部结构、五官、成年年龄、自然肤质与身体比例。
2. **连续叙事**：九张保持同一个核心场景、一套已确认妆造和一个连续事件状态。动作、手部、视线、姿态、景别与焦段仍按镜头职能变化，但空间、造型和事件后果必须严格连续。

章节式默认九镜依次使用约 24、28、35、40、50、65、85、105 和 135 毫米，并为每个焦段配置独立构图与透视目的。数字只是摄影意图，不是假装模型一定精确模拟某支镜头；每个提示词还会写明实际应当出现的空间关系和信息重点。
...
- 九个镜头是否具有九个不同叙事职能；
- 章节式方案的九个场景、九套妆造、九种身体状态、眼神面朝、构图、焦段和灯光是否真的逐镜可区分；或连续叙事中的单场景、单妆造和事件状态是否严格连续；
- 九个焦段是否各自服务于镜头职能，而不是只随机更换数字；
- 人物面部身份、成年年龄、自然肤质与身体比例是否锁定，同时允许逐镜改变妆容、发型和服装；
- 道具是否有明确所有者与状态变化；
- 每镜灯光是否有明确光源、方向、软硬、反差与衰减，并且符合该镜场景；
...
| 用途 | 允许影响 | 不应自动影响 |
|---|---|---|
| `identity` | 同一人物面部身份、成年年龄、稳定面部特征 | 姿势、动作、手势、眼神、面部朝向、表情、背景、机位、构图、妆容、发型、服装、焦段、灯光 |
...
最终 Prompt/Skill ZIP 会把经过元数据清理的真实参考图复制到 `assets/references/`，在 `references/reference_manifest.json` 中记录用途与权利，并在 `references/run_manifest.json` 中逐镜列出必须传入的图片。identity 参考图默认适用于九镜，但只能保持同一个人物面部身份；九镜提示词会明确禁止继承参考图的姿势、动作、手势、眼神、面部朝向、场景、构图、妆发服装、焦段、灯光和背景。
...
| Codex 开始向你解释结构化文件 | 确认插件为 0.16.0；对旧包直接说“打开这个 APSAL 包并生成第一张图” |
| 英文输入仍显示中文卡片 | 升级到 0.16.0 后打开新任务，或直接要求切换到英文 |
| 中文卡片仍出现英文内部字段 | 确认插件为 0.16.0 并打开新任务；新版中文呈现层会隐藏内部编号和机器标签 |
| 元素卡只显示标题或有空白区域 | 确认插件为 0.16.0 并新建任务；新版卡片应同时显示提案、理由、可点击方向、取值、预期结果、保持项和验收标准 |
| 五阶段没有缩略图 | 确认插件为 0.16.0 并打开新任务；DNA 选择卡保持纯文字，元素卡上方才显示五阶段缩略图 |
| Skill 有缩略图却没有真实参考图 | 查看 `reference_manifest.json`；`not_bound` 表示主题没有绑定真实图片，缩略图不能代替参考图 |
| 九张图总在重复同一场景、妆造、姿势或焦段 | 新建 0.15 项目并保留默认“章节式丰富变化”，或在第一张卡从“连续叙事”切换；确认九场景、九妆造、九种身体状态、九种眼神面朝、九构图、九焦段和九灯光计划后再继续 |
