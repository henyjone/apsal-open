...
```text
apsal-studio@apsal-open  installed, enabled  0.11.0
```
...
APSAL Studio 默认跟随当前 Codex 对话，不在安装后强制显示语言选择页。中文创作描述建立中文会话，英文创作描述建立英文会话。0.11.0 起，中文卡片只显示面向创作者的中文：内部角色名、层编号、字段名、状态、来源、资源层级、资源编号、质量状态和语义标签都留在后台，不再出现在卡片中。
...
从 0.9.0 开始不需要自己解压旧包；0.10.0 提供完整中英文操作说明；0.11.0 进一步统一中文卡片与重要文字高亮。把压缩包附到 Codex，输入：
...
| Codex 开始向你解释结构化文件 | 确认插件为 0.11.0；对旧包直接说“打开这个 APSAL 包并生成第一张图” |
| 英文输入仍显示中文卡片 | 升级到 0.11.0 后打开新任务，或直接要求切换到英文 |
| 中文卡片仍出现英文内部字段 | 确认插件为 0.11.0 并打开新任务；新版中文呈现层会隐藏内部编号和机器标签 |
