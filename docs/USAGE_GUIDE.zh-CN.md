...
```text
apsal-studio@apsal-open  installed, enabled  0.10.0
```
...
APSAL Studio 默认跟随当前 Codex 对话，不在安装后强制显示语言选择页。中文 brief 建立中文会话，英文 brief 建立英文会话；卡片标题、问题、按钮、解释和文字回退只显示当前语言，不再中英文并排混用。
...
从 0.9.0 开始不需要自己解压旧包；0.10.0 进一步提供完整中英文操作说明。把 ZIP 附到 Codex，输入：
...
| Codex 开始向你解释 JSON | 确认插件为 0.10.0；对旧包直接说“打开这个 APSAL 包并生成第一张图” |
| 英文输入仍显示中文卡片 | 升级到 0.10.0 后打开新任务，或直接说“use English”；会话语言应显示为 `en` |
