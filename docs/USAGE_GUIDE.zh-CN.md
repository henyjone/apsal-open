...
```text
apsal-studio@apsal-open  installed, enabled  0.12.0
```
...
APSAL Studio 默认跟随当前 Codex 对话，不在安装后强制显示语言选择页。中文创作描述建立中文会话，英文创作描述建立英文会话。0.12.0 的中文卡片只显示面向创作者的中文：内部角色名、层编号、字段名、状态、来源、资源层级、资源编号、质量状态和语义标签都留在后台，不再出现在卡片中。
...
人物没有被特别指定时，默认提案是一位气质鲜明、从容、镜头表现稳定的东亚成年女主角，并明确要求她能适配古典、当代、编辑、礼服与仪式感等多种妆发服装。这里的“多妆造适配”不等于人物可以漂移：面部结构、成年年龄、真实肤质、发色发际线和身体比例继续锁定；同一套片仍保持一套已确认妆造，只有事件明确推动时才允许变化。若创作描述明确要求成年男主角，插件会尊重该要求。
...
从 0.9.0 开始不需要自己解压旧包；0.10.0 提供完整中英文操作说明；0.11.0 统一中文卡片与重要文字高亮；0.12.0 将完整设计提案和验收标准都放入卡片。把压缩包附到 Codex，输入：
...
| Codex 开始向你解释结构化文件 | 确认插件为 0.12.0；对旧包直接说“打开这个 APSAL 包并生成第一张图” |
| 英文输入仍显示中文卡片 | 升级到 0.12.0 后打开新任务，或直接要求切换到英文 |
| 中文卡片仍出现英文内部字段 | 确认插件为 0.12.0 并打开新任务；新版中文呈现层会隐藏内部编号和机器标签 |
| 元素卡只显示标题或有空白区域 | 确认插件为 0.12.0 并新建任务；新版卡片应同时显示提案、理由、可调整方向、取值、预期结果、保持项和验收标准 |
