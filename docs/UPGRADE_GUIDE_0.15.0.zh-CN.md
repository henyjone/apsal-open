...
4. 预览记录 <code>origin</code>、<code>summary</code> 和逐字段 <code>changes</code>；
5. Studio 显示变更卡；
6. 确认时调用 <code>design.commit_preview(expectedRevision=13)</code>；
7. 提交成功后 revision 变为 14。
...
| apsal_frontend_get_updates | project.snapshot | 读取 Studio 发出的字段差异和下游影响 |
...
1. 属性：显示 Engine 管理的中文语义；编辑后点击“发送给 Codex”，创建带来源和字段差异的 preview，不直接覆盖项目。
...
6. Studio 修改上游元素并发送给 Codex。
7. Codex 收件箱读到同一字段差异，应用后两端看到相同下游 pending 状态。
